...
async function main() {
  ...
  if (args.length < 1 || args[0] === "--help" || args[0] === "-h") {
    console.log(
      "Usage: yarn ts-node scripts/update-treasury-authority.ts <NEW_TREASURY_AUTHORITY>"
    );
    console.log();
    console.log(
  ...
  const current = poolAccount.treasuryAuthority;

  console.log("=".repeat(60));
  console.log("UPDATE TREASURY AUTHORITY");
  console.log("=".repeat(60));
  console.log("- Program ID:", program.programId.toString());
  console.log("- Pool PDA:", pool.toString());
  console.log("- Current Treasury Authority:", current.toString());
  console.log("- Your Wallet:", payer.publicKey.toString());
  console.log("- New Treasury Authority:", next.toString());

  if (!current.equals(payer.publicKey)) {
    console.error("❌ Error: You are not the current treasury authority");
    process.exit(1);
  }
  if (current.equals(next)) {
    console.log("ℹ️  Treasury Authority is already set to this address");
    process.exit(0);
  }
  ...
  console.log("✅ Treasury Authority rotated.");

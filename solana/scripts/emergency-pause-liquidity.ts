...
async function main() {
  ...
  if (args[0] === "--help" || args[0] === "-h") {
    console.log(
      "Usage: yarn ts-node scripts/emergency-pause-liquidity.ts [PAUSED]"
    );
    console.log();
    console.log("Arguments:");
    console.log(
      "  PAUSED     true to pause liquidity operations, false to unpause (optional, defaults to true)"
    );
    console.log();
  ...
  if (poolAccount.liquidityPaused === paused) {
    console.log(
      `ℹ️  Liquidity operations are already ${paused ? "paused" : "unpaused"}`
    );
    process.exit(0);
  }

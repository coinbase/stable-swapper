...
async function main() {
  ...
  if (args.length < 1 || args[0] === "--help" || args[0] === "-h") {
    console.log(
      "Usage: yarn ts-node scripts/update-unpause-authority.ts <NEW_UNPAUSE_AUTHORITY>"
    );
    console.log();
    console.log(
  ...
  const current = poolAccount.unpauseAuthority;

  console.log("=".repeat(60));
  console.log("UPDATE UNPAUSE AUTHORITY");
  console.log("=".repeat(60));
  console.log("- Program ID:", program.programId.toString());
  console.log("- Pool PDA:", pool.toString());
  console.log("- Current Unpause Authority:", current.toString());
  console.log("- Your Wallet:", payer.publicKey.toString());
  console.log("- New Unpause Authority:", next.toString());

  if (!current.equals(payer.publicKey)) {
    console.error("❌ Error: You are not the current unpause authority");
    process.exit(1);
  }
  if (current.equals(next)) {
    console.log("ℹ️  Unpause Authority is already set to this address");
    process.exit(0);
  }
  ...
  console.log("✅ Unpause Authority rotated.");

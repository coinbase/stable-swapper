...
contract SwapTest is StableSwapperBase {
    ...
    function test_revertsWhenSwapsPaused() public {
    ...
    function test_revertsWhenSwappingSameToken() public {
    ...
    function test_revertsWhenInsufficientLiquidity() public {
        vm.startPrank(operationsAuthority);
        swapper.addToken(address(usdc));
        ...
        swapper.deposit_liquidity(address(appStable), 100 * 10 ** 6);
        vm.stopPrank();
    ...
    function test_revertsWhenSlippageProtectionTriggered() public {
        setupBasicSwapEnvironment();
    ...
    function test_revertsWhenSwapAmountResultsInZeroOutput() public {
        vm.startPrank(operationsAuthority);
        swapper.addToken(address(usdc));
        swapper.addToken(address(appStable));
    ...
    function test_revertsWhenInputTokenDisabled() public {
        setupBasicSwapEnvironment();
    ...
    function test_revertsWhenOutputTokenDisabled() public {
        setupBasicSwapEnvironment();
    ...
    function test_swapsUsdcForAppStable() public {
        setupBasicSwapEnvironment();
    ...
    function test_swapsAppStableForUsdc() public {
        setupBasicSwapEnvironment();
    ...
    function test_swapsFrom6DecimalsTo9DecimalsScalingUp() public {
        ...
        swapper.deposit_liquidity(address(token9Dec), 500 * 10 ** 9);
        vm.stopPrank();
        ...
        assertEq(usdc.balanceOf(wallet0), initialUserUsdc - swapAmount);
        assertEq(token9Dec.balanceOf(wallet0), 100 * 10 ** 9);
    }
    
    function test_swapsFrom9DecimalsTo6DecimalsScalingDown() public {
        MockERC20 token9Dec = new MockERC20("9 Decimal Token", "TOK9", 9);
        token9Dec.mint(operationsAuthority, 1000 * 10 ** 9);
        ...
        assertEq(usdc.balanceOf(wallet0), 1000 * 10 ** 6 + 100 * 10 ** 6);
    }
    
    function test_properlyRoundsDownWhenScalingFrom9To6Decimals() public {
        MockERC20 token9Dec = new MockERC20("9 Decimal Token", "TOK9", 9);
        token9Dec.mint(operationsAuthority, 1000 * 10 ** 9);
        token9Dec.mint(wallet0, 1000 * 10 ** 9);
        ...
        // Should receive exactly 100_000_000 (rounded down)
        assertEq(usdc.balanceOf(wallet0), initialUserUsdc + 100_000_000);
    }
    
    function test_swapsWithSameDecimalsWork() public {
        setupBasicSwapEnvironment();
    ...
    function test_respectsReservedAmountInSwaps() public {
        vm.startPrank(operationsAuthority);
        swapper.addToken(address(usdc));
        swapper.addToken(address(appStable));
    ...
    function test_collectsFeesCorrectlyWhenFeeRateNonZero() public {
        setupBasicSwapEnvironment();
    ...
    function test_skipsFeesWhenFeeRateIsZero() public {
        setupBasicSwapEnvironment();
    ...
    function test_roundsUpFeesWith99999Amount() public {
        setupBasicSwapEnvironment();
        ...
        assertEq(feeCollected, 1000, "Fee should round up to 1000 units from 999.99 units");
    ...
    function test_roundsUpFeesToPreventProtocolLoss() public {
        setupBasicSwapEnvironment();
        ...
        vm.startPrank(wallet0);
        usdc.approve(address(swapper), swapAmount);
        swapper.swap(address(usdc), address(appStable), swapAmount, 9900, wallet0);
        vm.stopPrank();
        
        uint256 feeCollected = usdc.balanceOf(feeRecipient) - initialFeeRecipientBalance;
        assertEq(feeCollected, 101); // Rounded up from 100.5
    ...
    function test_doesNotOverChargeOnPerfectFeeAmounts() public {
        setupBasicSwapEnvironment();
        ...
        vm.startPrank(wallet0);
        usdc.approve(address(swapper), swapAmount);
        swapper.swap(address(usdc), address(appStable), swapAmount, 99 * 10 ** 6, wallet0);
        vm.stopPrank();
        
        uint256 feeCollected = usdc.balanceOf(feeRecipient) - initialFeeRecipientBalance;
        assertEq(feeCollected, 1 * 10 ** 6); // Exactly 1%, not rounded up
